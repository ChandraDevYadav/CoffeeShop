  ...
  return (
    <div className="mt-36">
      ...
      <Swiper
        ...
        className="mx-28 my-16"
      >
        {swiperData.map((item) => (
          <SwiperSlide key={item.id}>
            <div className="flex flex-col items-start pb-20 ">
              <img src={item.image} alt={item.name} className="shadow-lg" />
              <div className="bg-white pl-4 w-[18rem] shadow-lg py-10">
                <h3 className="text-[1.8rem] font-caudex font-semibold text-start">
                  {item.name}
                </h3>
                <div className="flex gap-4">
                  <span className="text-red-500 text-2xl font-caudex font-medium line-through mr-2">
                    {item.oldPrice}
                  </span>
                  <span className="text-green-500 text-2xl font-caudex font-medium">
                    {item.newPrice}
                  </span>
                </div>
              </div>
